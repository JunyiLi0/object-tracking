...
# Load a pre-trained mobilenet_v2
...
class Track:
    def __init__(self, detection, track_id, image):
        self.detections = [detection]
        self.track_id = track_id
        self.age = 0
        self.kalman_filter = KalmanFilter(dt=0.1, u_x=1, u_y=1, std_acc=1, x_std_meas=0.1, y_std_meas=0.1)
    ...
    def extract_features(self, image):
        x, y, w, h = map(int, self.detections[-1])
    ...
    def update(self, detections, image):
        if len(self.tracks) == 0:
            for det in detections:
                self.tracks.append(Track(det, self.next_id, image))
                self.next_id += 1
        else:
            ...
            for match in matches:
                self.tracks[match[1]].update(detections[match[0]], image)

            for index in unmatched_detections:
                self.tracks.append(Track(detections[index], self.next_id, image))
                self.next_id += 1
...
for i, image_path in enumerate(image_paths):
    ...
    # Update the tracker
    tracker.update(frame_detections, image)
